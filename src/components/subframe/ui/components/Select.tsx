  ...
  return (
    <SubframeCore.Select.Item value={value as string} asChild={true} {...otherProps}>
      <div
        ...
        ref={ref as any}
      >
        <Select.ItemText className="h-auto shrink-0 grow basis-0">{children || value}</Select.ItemText>
        <SubframeCore.Icon
          className="hidden font-body text-body text-default-font group-hover/969e345b:hidden group-data-[state=checked]/969e345b:inline-flex group-data-[state=checked]/969e345b:text-brand-600"
          name="FeatherCheck"
        />
      </div>
...
const TriggerValue = React.forwardRef<HTMLElement, TriggerValueProps>(function TriggerValue(
...
interface TriggerProps extends Omit<React.ComponentProps<typeof SubframeCore.Select.Trigger>, "placeholder"> {
  placeholder?: React.ReactNode;
...
const Trigger = React.forwardRef<HTMLElement, TriggerProps>(function Trigger(
  ...
  return (
    <SubframeCore.Select.Trigger asChild={true} {...otherProps}>
      <div
        className={SubframeUtils.twClassNames("flex h-full w-full items-center gap-2 px-3", className)}
        ref={ref as any}
      >
        <SubframeCore.Icon className="font-body text-body text-neutral-400" name={icon} />
        <Select.TriggerValue placeholder={placeholder as string} />
        <SubframeCore.Icon className="font-body text-body text-subtext-color" name="FeatherChevronDown" />
      </div>
...
const ItemText = React.forwardRef<HTMLElement, ItemTextProps>(function ItemText(
  ...
  return children ? (
    <SubframeCore.Select.ItemText {...otherProps}>
      <span className={SubframeUtils.twClassNames("font-body text-body text-default-font", className)} ref={ref as any}>
        {children}
      </span>
...
const SelectRoot = React.forwardRef<HTMLElement, SelectRootProps>(function SelectRoot(
  {
    disabled = false,
    error = false,
    variant = "outline",
  ...
  return (
    ...
    >
      <div
        ...
        {...otherProps}
      >
        {label ? <span className="font-caption-bold text-caption-bold text-default-font">{label}</span> : null}
        <div
          className={SubframeUtils.twClassNames(
        ...
        {helpText ? (
          <span
            className={SubframeUtils.twClassNames("font-caption text-caption text-subtext-color", {
        ...
        <Content>
          {children ? <div className="flex w-full shrink-0 grow basis-0 flex-col items-start">{children}</div> : null}
        </Content>
      </div>
    </SubframeCore.Select.Root>
  );
