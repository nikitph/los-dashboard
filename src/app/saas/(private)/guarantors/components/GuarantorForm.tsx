...
interface GuarantorFormProps {
  initialData?: Partial<GuarantorFormValues> & { id?: string };
  isEditMode?: boolean;
}

export default function GuarantorForm({ initialData, isEditMode = false }: GuarantorFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      Object.keys(initialData).forEach((key) => {
        const typedKey = key as keyof GuarantorFormValues;
  ...
  const onSubmit = async (data: GuarantorFormValues) => {
    ...
    try {
      ...
      if (isEditMode && initialData?.id) {
        response = await updateGuarantor(initialData.id, data);
      } else {
        response = await createGuarantor(data);
      }

      if (response.success) {
        toast({
          title: isEditMode ? "Guarantor Updated" : "Guarantor Created",
  ...
  return (
    <div className="w-full bg-transparent">
      <div className="mx-auto max-w-3xl">
        <h2 className="mb-6 text-2xl font-semibold">{isEditMode ? "Edit Guarantor" : "Add New Guarantor"}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex items-center gap-4 pt-4">
            <Button type="button" variant="outline" onClick={() => router.push("/saas/guarantors/list")}>
              Cancel
            </Button>
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? "Processing..." : isEditMode ? "Update Guarantor" : "Create Guarantor"}
            </Button>
          </div>
        </form>
      </div>
