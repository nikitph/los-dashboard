...
import { VerificationFormValues } from "@/app/[locale]/saas/(private)/verifications/schemas/verificationSchema";
...
/**
 * Props for the PropertyVerificationFormFields component
 */
interface PropertyVerificationFormFieldsProps {
  /**
...
/**
 * Form fields for property verification with field-level permission control
 *
 * @param {PropertyVerificationFormFieldsProps} props - Component props
 * @returns {JSX.Element} The property verification form fields
 */
const PropertyVerificationFormFields: React.FC<PropertyVerificationFormFieldsProps> = ({
  form,
  visibility,
  formattedDate,
  user,
  ...
  return (
    <>
      {/* Hidden field for loan application ID */}
      <input type="hidden" {...form.register("verification.loanApplicationId")} value={loanApplicationId} />
        ...
        <div className="flex w-full flex-col items-start gap-2">
          ...
          <div className="flex w-full items-start gap-4">
            ...
            {(visibility.addressState || visibility.canUpdateAddressState) && (
              <FormField
                control={form.control}
                name="propertyVerification.addressState"
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      {visibility.canUpdateAddressState ? (
            ...
            {(visibility.addressCity || visibility.canUpdateAddressCity) && (
              <FormField
                control={form.control}
                name="propertyVerification.addressCity"
                render={({ field }) => (
                  <FormItem className="w-full">
                    <FormControl>
                      {visibility.canUpdateAddressCity ? (
            ...
            {(visibility.addressZipCode || visibility.canUpdateAddressZipCode) && (
              <FormField
                control={form.control}
                name="propertyVerification.addressZipCode"
                render={({ field }) => (
                  <FormItem className="w-full">
                    <FormControl>
                      {visibility.canUpdateAddressZipCode ? (
          ...
          {(visibility.addressLine1 || visibility.canUpdateAddressLine1) && (
            <FormField
              control={form.control}
              name="propertyVerification.addressLine1"
              render={({ field }) => (
                <FormItem className="w-full">
                  <FormControl>
                    {visibility.canUpdateAddressLine1 ? (
          ...
          {(visibility.addressLine2 || visibility.canUpdateAddressLine2) && (
            <FormField
              control={form.control}
              name="propertyVerification.addressLine2"
              render={({ field }) => (
                <FormItem className="w-full">
                  <FormControl>
                    {visibility.canUpdateAddressLine2 ? (
                      <TextField className="h-auto w-full flex-none" label="" helpText="">
                        <TextField.Input placeholder={t("form.fields.addressLine2.placeholder")} {...field} />
                      </TextField>
      ...
      {(visibility.locationFromMain || visibility.canUpdateLocationFromMain) && (
        <FormField
          control={form.control}
          name="propertyVerification.locationFromMain"
          render={({ field }) => (
            <FormItem className="w-full">
              <FormLabel className="font-body-bold text-body-bold text-default-font">
      ...
      {shouldShowOwnerSection() && (
        <div className="flex w-full flex-col items-start gap-4">
          <div className="flex w-full items-start gap-4">
            <div className="flex shrink-0 grow basis-0 flex-col items-start gap-2">
              <span className="font-body-bold text-body-bold text-default-font">{t("view.fields.property.owner")}</span>
              <div className="flex w-full items-center gap-4">
                {/* Owner First Name */}
                {(visibility.ownerFirstName || visibility.canUpdateOwnerFirstName) && (
                  <FormField
                    control={form.control}
                    name="propertyVerification.ownerFirstName"
                    render={({ field }) => (
                      <FormItem className="w-full">
                        <FormControl>
                          {visibility.canUpdateOwnerFirstName ? (
                            <TextField className="h-auto w-full flex-none" label="" helpText="">
                              <TextField.Input
                                placeholder={t("form.fields.property.ownerFirstName.placeholder")}
                                {...field}
                              />
                            </TextField>
                          ) : visibility.ownerFirstName ? (
                ...
                {(visibility.ownerLastName || visibility.canUpdateOwnerLastName) && (
                  <FormField
                    control={form.control}
                    name="propertyVerification.ownerLastName"
                    render={({ field }) => (
                      <FormItem className="w-full">
                        <FormControl>
                          {visibility.canUpdateOwnerLastName ? (
                            <TextField className="h-auto w-full flex-none" label="" helpText="">
                              <TextField.Input
                                placeholder={t("form.fields.property.ownerLastName.placeholder")}
                                {...field}
                              />
                            </TextField>
                          ) : visibility.ownerLastName ? (
          ...
          {(visibility.structureType || visibility.canUpdateStructureType) && (
            <div className="flex w-1/2 items-start gap-4 pe-2">
              <FormField
                control={form.control}
                name="propertyVerification.structureType"
                render={({ field }) => (
                  <FormItem className="w-full">
                    <FormLabel className="font-body-bold text-body-bold text-default-font">
                      {t("form.fields.property.structureType.label")}
                    </FormLabel>
                    <FormControl>
                      {visibility.canUpdateStructureType ? (
                        <Select
                          ...
                          value={field.value}
                          placeholder={
                            <div className="text-gray-600">{t("form.fields.property.structureType.placeholder")}</div>
                          }
                          className="w-full"
                          variant="outline"
                        >
                          <Select.Item key="Commercial" value="Commercial">
                            {t("form.fields.property.structureType.options.commercial")}
                          </Select.Item>
                          <Select.Item key="Residential" value="Residential">
                            {t("form.fields.property.structureType.options.residential")}
                          </Select.Item>
                          <Select.Item key="Industrial" value="Industrial">
                            {t("form.fields.property.structureType.options.industrial")}
                          </Select.Item>
                          <Select.Item key="Mixed" value="Mixed">
                            {t("form.fields.property.structureType.options.mixed")}
                          </Select.Item>
                        </Select>
                      ) : visibility.structureType ? (
                        <div className="text-gray-700">
                          {field.value
                            ? t(`form.fields.property.structureType.options.${field.value.toLowerCase()}`)
                            : "-"}
                        </div>
                      ) : null}
                    </FormControl>
...
export default PropertyVerificationFormFields;
